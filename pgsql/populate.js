...
module.exports = function () {
    ...
    const mail = '@fortesting.com';
    
  
    for (let i = 0; i < quantity; i++) {
        const email = adjectives[random(adjectives.length)] +
        nouns[random(nouns.length)] + mail;
        const lessons = [...new Array(random(100,1))].map((data, index) => {
            return index;
        });
        const newUser = {
            email: email,
        ...
        lessonName = `How to ${verbs[random(verbs.length)]} ${nouns[random(nouns.length)]}s`;
        lessonDescription = `A journey into the ${adjectives[random(adjectives.length)]} ${nouns[random(nouns.length)]}s of ${countries[random(countries.length)]}`;
        const newLesson = {
            name: lessonName,
